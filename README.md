...
- v3.1.0: ZIP 구조 수정 (qmslide/ 래퍼 폴더 추가로 설치 경로 오인식 문제 해결)
...
# 3) _qm_extract 안의 qmslide 폴더를 스킬 경로로 이동
# [!] ZIP 압축 해제 시 _qm_extract 안에 qmslide/ 폴더가 자동 생성됩니다
Move-Item "$env:USERPROFILE\.claude\skills\_qm_extract\qmslide" `
          "$env:USERPROFILE\.claude\skills\qmslide"
...
# 3) qmslide 폴더를 스킬 경로로 이동
# [!] ZIP 압축 해제 시 qmslide/ 폴더가 자동 생성됩니다
mv /tmp/_qm_extract/qmslide ~/.claude/skills/qmslide

# 4) 임시 폴더 및 zip 파일 정리
rm -rf /tmp/_qm_extract
