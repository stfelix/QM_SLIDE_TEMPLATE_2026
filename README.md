...
1. `Settings` → `Capabilities` → **Code execution** 켜기
2. 좌측 사이드바 `Customize` → `Skills` 탭 클릭
3. **`+` 버튼** → `Upload skill` 선택
4. 다운로드한 `quad-miners-pptx-skill.skill` 파일 업로드 (1~3분 소요)
5. My Skills 목록에 **quad-miners-pptx** 표시되면 완료
...
1. PowerPoint 열기
2. **Home 탭** → `Add-ins` 클릭  
   *(Mac: 메뉴바 `Tools` → `Add-ins`)*
3. 검색창에 **`Claude by Anthropic`** 입력 → **Add** 클릭
...
1. [Microsoft 365 Admin Center](https://admin.microsoft.com) → `Settings` → `Integrated Apps`
2. 검색창에 **`Claude by Anthropic`** 입력
3. `Deploy` → 배포 대상(전체 조직 또는 그룹) 선택 후 확인
...
3. 우측 Claude 사이드바 → `Skills` 탭 → **quad-miners-pptx** 선택
4. 또는 사이드바 하단 `Instructions` 필드에 Quad Miners 브랜드 가이드 붙여넣기
...
| Instructions | 브랜드 지침·선호도를 사이드바에 영구 저장 |
